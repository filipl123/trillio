...
gulp.task('clean', function(cb) {
  return del([
    'public/build/**/*', '!public/build/bower_components'
    ], cb);
});
...
// Clean jsons
gulp.task('clean:jsons', function(cb) {
  return del([
    'public/build/jsons/**'
  ], cb);
});
...
gulp.task('copy:json', ['clean:jsons'], function () {
  var json_dest = ['public/src/jsons/**/*.json'];
  return gulp.src(json_dest)
  .pipe(plumber())
    .pipe(gulp.dest('public/build/jsons'))
    .pipe(livereload());
});
...
// Build task
gulp.task('build', ['js', 'html', 'copy:json', 'icons', 'fonts', 'images', 'copy:png', 'copy:filters', 'copy:favicon', 'sass']);
